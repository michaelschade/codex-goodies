...
- **Teach Codex to link back into itself.** It now covers the desktop `codex://` deeplink shapes that matter for opening settings, skills, automations, or a new prefilled chat, so Codex can hand back actionable links instead of only describing where to click.
