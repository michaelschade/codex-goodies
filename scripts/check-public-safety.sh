#!/usr/bin/env bash
set -euo pipefail

ROOT_DIR="$(CDPATH= cd -- "$(dirname -- "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT_DIR"

failures=0

fail() {
  printf '[FAIL] %s\n' "$1" >&2
  failures=$((failures + 1))
}

pass() {
  printf '[OK] %s\n' "$1" >&2
}

require_git() {
  command -v git >/dev/null 2>&1 || {
    printf 'git is required for %s\n' "${BASH_SOURCE[0]}" >&2
    exit 1
  }
}

check_tracked_path_banlist() {
  local banned=''

  banned=$(git ls-files | rg '(^|/)(auth\.json|session_index\.jsonl|models_cache\.json|installation_id|version\.json|\.codex-global-state\.json|.*\.sqlite(|-shm|-wal)|.*\.db|.*\.db-shm|.*\.db-wal|.*\.bak|.*\.orig|.*~|bak-skill\.md)$|(^|/)(archived_sessions|automations|cache|generated_images|log|memories|sessions|shell_snapshots|sqlite)/|^codex-home/skills/(\.system|codex-primary-runtime)(/|$)' || true)
  if [[ -n "$banned" ]]; then
    fail "tracked files or directories from local Codex state are present:"
    printf '%s\n' "$banned" >&2
  else
    pass "no banned local-state paths are tracked"
  fi
}

check_absolute_home_paths() {
  local matches=''

  matches=$(rg -n '/Users/[A-Za-z0-9_-][^[:space:]"]+|/home/[A-Za-z0-9_-][^[:space:]"]+' AGENTS.md CONTRIBUTING.md README.md codex-home .github scripts || true)
  if [[ -n "$matches" ]]; then
    fail "absolute home-directory paths were found in tracked content:"
    printf '%s\n' "$matches" >&2
  else
    pass "no absolute home-directory paths were found"
  fi
}

check_secret_like_content() {
  local matches=''

  matches=$(rg -n --glob '!scripts/check-public-safety.sh' 'sk-[A-Za-z0-9]{20,}|ghp_[A-Za-z0-9]{20,}|github_pat_[A-Za-z0-9_]{20,}|xox[baprs]-[A-Za-z0-9-]{10,}|BEGIN [A-Z ]*PRIVATE KEY|OPENAI_API_KEY=' AGENTS.md CONTRIBUTING.md README.md codex-home .github scripts || true)
  if [[ -n "$matches" ]]; then
    fail "secret-like content was found:"
    printf '%s\n' "$matches" >&2
  else
    pass "no secret-like content was found"
  fi
}

check_skill_shape() {
  local skill_dir=''

  while IFS= read -r skill_dir; do
    [[ -f "$skill_dir/SKILL.md" ]] || fail "skill is missing SKILL.md: $skill_dir"
  done < <(find codex-home/skills -mindepth 1 -maxdepth 1 -type d ! -name '.*' | LC_ALL=C sort)

  pass "every tracked custom skill has a SKILL.md entrypoint"
}

check_python_hook_compiles() {
  if [[ -f codex-home/hooks/session_start_subagents.py ]]; then
    python3 -m py_compile codex-home/hooks/session_start_subagents.py
    pass "hook script compiles with python3"
  fi
}

main() {
  require_git
  check_tracked_path_banlist
  check_absolute_home_paths
  check_secret_like_content
  check_skill_shape
  check_python_hook_compiles

  if ((failures > 0)); then
    printf '\npublic-safety check failed with %d issue(s)\n' "$failures" >&2
    exit 1
  fi

  printf '\npublic-safety check passed\n' >&2
}

main "$@"
